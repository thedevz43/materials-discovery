...
def train_epoch(model, loader, optimizer, device):
    ...
    for batch in loader:
        ...
        # Compute masked loss for each target
        loss = 0
        for i, target_name in enumerate(["volume", "band_gap", "formation_energy"]):
            target = batch.y[:, i]
            loss += masked_mae(out[:, i], target)
...
def masked_mae(pred, target):
    """
    Compute masked Mean Absolute Error, ignoring NaN values.
